...
class FieldServiceMobileApi(http.Controller):
            ...
            return self._json(
                {
                    "success": True,
                    "jobs": jobs,
                    "message": "No assigned Field Service Jobs found for this technician." if not jobs else "",
                }
            )
        except Exception as exc:
            return self._failure(exc, status=401 if isinstance(exc, AccessError) else 500)

    @http.route("/api/mobile/service-reports", type="http", auth="none", methods=["GET"], csrf=False)
    def service_reports(self):
        try:
            env = self._user_env()
            reports = [
                self._report_payload(report, full=True)
                for report in self._assigned_reports(env)
            ]
            return self._json(
                {
                    "success": True,
                    "reports": reports,
                    "message": "No Service Reports found for this technician." if not reports else "",
                }
            )
        except Exception as exc:
            return self._failure(exc, status=401 if isinstance(exc, AccessError) else 500)
    ...
    @http.route(
        "/api/mobile/service-reports/<int:report_id>/start",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
            ...
            return self._success(
                {
                    "report": self._report_payload(report, full=True),
                    "sync_status": "synced",
                    "message": "Service report started successfully",
                }
            )
        except Exception as exc:
            return self._failure(exc, status=400)
    ...
    @http.route(
        "/api/mobile/service-reports/<int:report_id>/stop",
        type="http",
        auth="none",
        methods=["POST"],
        csrf=False,
    )
    def stop_report(self, report_id):
        try:
            env = self._user_env()
            report = self._assigned_report(env, report_id)
            if not report:
            ...
            if report.task_id and "fsm_done" in report.task_id._fields:
                report.task_id.write({"fsm_done": True})
            return self._success(
                {
                    "report": self._report_payload(report, full=True),
                    "sync_status": "synced",
                    "message": "Service report stopped successfully",
                }
            )
        except Exception as exc:
            return self._failure(exc, status=400)
    ...
    def _report_assigned_to_user(self, report, user):
        if report.driver_id == user:
            return True
        if report.create_uid == user:
            return True
        if "assigned_user_id" in report._fields and report.assigned_user_id == user:
            return True
    ...
    def _report_values(self, env, data):
        job_id = data.get("field_service_job_id") or data.get("job_id")
        job = env["project.task"].browse(job_id).exists() if job_id else False
        ...
        values = {
            ...
            "ups_system_down": "Yes" if data.get("system_down", data.get("ups_system_down")) else "No",
            ...
            "customer_signature": data.get("customer_signature_base64") or data.get("customer_signature"),
            "driver_signature": data.get("technician_signature_base64") or data.get("technician_signature"),
            "customer_signature_name": data.get("customer_name"),
            "driver_signature_name": data.get("technician_name") or env.user.name,
            "mobile_external_id": data.get("mobile_external_id"),
            "mobile_device_id": data.get("mobile_device_id"),
            "mobile_sync_status": "synced",
            "mobile_last_sync_date": fields.Datetime.now(),
        }
        if arrival_dt:
    ...
    def _report_payload(self, report, full=False):
        base = {
            ...
            "field_service_job_id": report.task_id.id if report.task_id else None,
            "mobile_external_id": report.mobile_external_id,
            "source": report.source,
            "submitted_from_mobile": report.submitted_from_mobile,
            "mobile_sync_status": report.mobile_sync_status,
            "mobile_last_sync_date": fields.Datetime.to_string(report.mobile_last_sync_date) if report.mobile_last_sync_date else "",
            "is_emergency_report": report.is_emergency_report,
            "state": report.state,
            "labor_hours": report.labor_hours,
        }
        if not full:
